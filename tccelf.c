...
#ifdef TCC_TARGET_PE
#include <tlhelp32.h>
//FIXME: MEM_DEUBG failed, Here we make memtest failed, But in fact this is not a memory leak.
static HMODULE *windowsModules=NULL;
static int windowsModulesCount=0;
    ...
    dynarray_add(&windowsModules,&windowsModulesCount,LoadLibrary("msvcrt.dll"));
     ...
     if (handle != INVALID_HANDLE_VALUE)
     {
         ...
         if (Module32First(handle, &module))
         {
             do
             {
                dynarray_add(&windowsModules,&windowsModulesCount,module.hModule);
...
ST_FUNC void relocate_syms(TCCState *s1, Section *symtab, int do_resolve) {
  ...
  for_each_elem(symtab, 1, sym, ElfW(Sym)) {
    ...
    void *addr=windowsDlsym(&name[s1->leading_underscore]);
#else
  ...
  if (sr->sh_flags & SHF_ALLOC) {
    ...
    if (s1->output_type & TCC_OUTPUT_DYN) {
      ...
      if (r && 0 == (s->sh_flags & SHF_WRITE))
        tcc_warning("%d relocations to ro-section %s",
                    (unsigned)(r / sizeof *qrel), s->name);
#endif
    }
  }
#endif
...
static int elf_output_file(TCCState *s1, const char *filename) {
  ...
  ret = tcc_write_elf_file(s1, filename, dyninf.phnum, dyninf.phdr, file_offset,
                           sec_order);
the_end:
  tcc_free(sec_order);
...
#endif
        sh->sh_type != SHT_NOTE && sh->sh_type != SHT_NOBITS &&
        sh->sh_type != SHT_PREINIT_ARRAY && sh->sh_type != SHT_INIT_ARRAY &&
